"""" This module contains functions for generating plots. """
...
def plot_double_general_info(df, ISIN1, ISIN2):
    """
    Generate a formatted table with general information for
    two given ISINs in the DataFrame.
    ...
    Returns:
    - str: Formatted table with general information
    for the specified ISINs.
    """
...
def plot_rating_gauges(df, ISIN1, ISIN2, rating_name="Fitch"):
    """
    Generate side-by-side rating gauge plots for two specified
    ISINs based on the selected credit rating agency.

    Parameters:
    - df (pd.DataFrame): DataFrame containing bond data.
    - ISIN1 (str): The ISIN code of the initial bond.
    - ISIN2 (str): The ISIN code of the recommended bond.
    - rating_name (str, optional): The credit rating agency name
    (default is 'Fitch').
    ...
    Note:
    - The rating_name parameter can be set to 'Fitch', 'Moodys', or
    'SP' to visualize ratings from Fitch, Moody's, or Standard & Poor's.
    """
...
def plot_double_clients_repartition(df, ISIN_ex1, ISIN_ex2):
    """
    Generate side-by-side pie charts depicting the distribution
    of clients for two specified ISINs.
    ...
    client_count_df1["count_perc"] = np.round(
        client_count_df1["count"] * 100 / total_count1, 2
    )
    ...
    line_to_add1 = pd.DataFrame(
        {"company_short_name": ["Others"], "count_perc": [count_perc_other1]}
    )
    client_count_df1 = pd.concat(
        [client_count_df1, line_to_add1], ignore_index=True, axis=0
    )
    ...
    client_count_df2["count_perc"] = np.round(
        client_count_df2["count"] * 100 / total_count2, 2
    )
    ...
    line_to_add2 = pd.DataFrame(
        {"company_short_name": ["Others"], "count_perc": [count_perc_other2]}
    )
    client_count_df2 = pd.concat(
        [client_count_df2, line_to_add2], ignore_index=True, axis=0
    )
    ...
    plt.legend(
        client_count_df1["company_short_name"],
    ...
    plt.legend(
        client_count_df2["company_short_name"],
        bbox_to_anchor=(0.95, 0.95),
        loc="upper left",
        title="Company Names",
    )
...
def plot_double_price_evolution(df, ISIN1, ISIN2):
    ...
    plt.plot(
        ...
        marker="o",
        markersize=4,
        label=f"Initial ISIN {ISIN1} (in {df_ISIN1_currency})",
    )
    plt.plot(
        df_ISIN2["Deal_Date"],
        df_ISIN2["B_Price"],
        ...
        marker="o",
        markersize=4,
        label=f"Recommended ISIN {ISIN2} (in {df_ISIN2_currency})",
    )
